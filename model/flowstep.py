...
class FlowStep(InvertibleModule):
    '''
    FlowStep combines affine coupling, permutation, and global affine transformation
    ...
    def __init__(self, dims_in, dims_c=[],
        ...
        Args:
          ...
          double_subnets:
            apply 2 subnets alternately for the first half and second half.
          affine_clamping:
        ...
        # print("all dims except batch-dim:", self.sum_dims)
        if len(dims_c) == 0:
        ...
        #print(f"split_len1: {split_len1}")
        #print(f"split_len2: {split_len2}")

        self.splits = [split_len1, split_len2]
        #print(f"split: {self.splits}")
        ...
        if global_affine_type == 'SIGMOID':
            ...
            self.global_scale_activation = (
                lambda a: 10 * torch.sigmoid(a - 2.))
        elif global_affine_type == 'SOFTPLUS':
            global_scale = 2. * \
                np.log(np.exp(0.5 * 10. * global_affine_init) - 1)
        ...
        self.global_scale = nn.Parameter(torch.ones(
            1, self.in_channels, *([1] * self.input_rank)) * float(global_scale))
        self.global_offset = nn.Parameter(torch.zeros(
            1, self.in_channels, *([1] * self.input_rank)))
        ...
        if self.householder:
            ...
            self.vk_householder = nn.Parameter(
                0.2 * torch.randn(self.householder, channels), requires_grad=True)
            self.w_perm = None
        ...
        self.double_subnets = double_subnets
        if not double_subnets:
          self.subnet = subnet_constructor(
              self.splits[0] + self.condition_channels, 2 * self.splits[1], ecaa_kernel_size=3)
          
          # print(
          # "subnet_constructor Param#: {}".format(
          #     sum(p.numel() for p in self.subnet.parameters() if p.requires_grad)
          #     )
          # )

        else:
          self.subnet_1 = subnet_constructor(
            self.splits[0] + self.condition_channels, 2 * self.splits[1], ecaa_kernel_size=1)

          self.subnet_2 = subnet_constructor(
            self.splits[1] + self.condition_channels, 2 * self.splits[0], ecaa_kernel_size=3)
      
          # print(
          # "subnet_constructor Param#: {}".format(
          #     sum(p.numel() for p in self.subnet_1.parameters() if p.requires_grad)*2
          #     )
          # )
    ...
    def _construct_householder_permutation(self):
        ...
        for vk in self.vk_householder:
            w = torch.mm(w, torch.eye(self.in_channels).to(
                w.device) - 2 * torch.ger(vk, vk) / torch.dot(vk, vk))
    ...
    def _affine(self, x, a, rev=False):
        ...
        # print("t:", len(sub_jac), len(sub_jac[0]), len(sub_jac[0][0]), len(sub_jac[0][0][0]))
    ...
    def forward(self, x, c=[], rev=False, jac=True):
        ...
        # Single subnet
        if not self.double_subnets:
          if not rev:
              x1_c = torch.cat([x1, *c], 1) if self.conditional else x1
              a1 = self.subnet(x1_c)
              x2_, j2 = self._affine(x2, a1)
          ...
          log_jac_det = j2
          x_out = torch.cat((x1, x2_), 1)

        else:
          # Double subnets
          if not rev:
              x1_c = torch.cat([x1, *c], 1) if self.conditional else x1
              a1 = self.subnet_1(x1_c)
              y2, j2 = self._affine(x2, a1)

              y2_c = torch.cat([y2, *c], 1) if self.conditional else y2
              a2 = self.subnet_2(y2_c)
              y1, j1 = self._affine(x1, a2)
          else:
              # names of x and y are swapped for the reverse computation
              x2_c = torch.cat([x2, *c], 1) if self.conditional else x2
              a2 = self.subnet_2(x2_c)
              y1, j1 = self._affine(x1, a2, rev=True)

              y1_c = torch.cat([y1, *c], 1) if self.conditional else y1
              a1 = self.subnet_1(y1_c)
              y2, j2 = self._affine(x2, a1, rev=True)
          log_jac_det = j1 + j2
          x_out = torch.cat((y1, y2), 1)
